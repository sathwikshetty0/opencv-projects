    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                finger_count = sum([thumb.x < lm[3].x, index.y < lm[6].y, middle.y < lm[10].y, ring.y < lm[14].y, pinky.y < lm[18].y]); reactions = ["😐", "😀", "😂", "😮", "🤩", "🙌"]; cv2.putText(output, reactions[min(finger_count,5)], (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,255,255), 2)

            cv2.putText(output, 'Display emoji reactions from hand pose', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('57 Hand Emoji Reactions', output)
