    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                brightness = np.interp(index_pos[1], [0, h], [2.0, 0.5]); output = cv2.convertScaleAbs(output, alpha=brightness, beta=0); cv2.putText(output, f"Brightness: {brightness:.2f}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)

            cv2.putText(output, 'Adjust screen brightness by finger height', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('51 Hand Brightness Slider', output)
