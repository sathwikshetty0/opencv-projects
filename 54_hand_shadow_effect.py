    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                shadow = output.copy(); cv2.circle(shadow, (midpoint[0]+20, midpoint[1]+20), 80, (0,0,0), -1); output = cv2.addWeighted(output, 0.75, shadow, 0.25, 0);

            cv2.putText(output, 'Render a dynamic shadow from the hand outline', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('54 Hand Shadow Effect', output)
