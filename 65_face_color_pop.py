    ...
    with mp_face.FaceMesh(max_num_faces=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as face_mesh:
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                hsv = cv2.cvtColor(output, cv2.COLOR_BGR2HSV); mask = cv2.inRange(hsv, (0, 40, 40), (20, 255, 255)); gray = cv2.cvtColor(output, cv2.COLOR_BGR2GRAY); gray = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR); output = np.where(mask[:,:,None] == 255, output, gray)

            cv2.putText(output, 'Pop one facial color while desaturating the rest', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('65 Face Color Pop', output)
