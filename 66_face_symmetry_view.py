    ...
    with mp_face.FaceMesh(max_num_faces=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as face_mesh:
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                mid = w//2; left = output[:, :mid]; right = cv2.flip(left, 1); output[:, mid:] = right

            cv2.putText(output, 'Visualize facial symmetry with a split view', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('66 Face Symmetry View', output)
