    ...
    with mp_face.FaceMesh(max_num_faces=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as face_mesh:
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                hsv = cv2.cvtColor(output, cv2.COLOR_BGR2HSV); hsv[:,:,1] = cv2.add(hsv[:,:,1], 50); output = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

            cv2.putText(output, 'Blend two face filters along the nose line', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('62 Face Blend Filter', output)
