    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                for i in range(0, 360, 45): cv2.circle(output, (midpoint[0]+int(40*np.cos(np.radians(i))), midpoint[1]+int(40*np.sin(np.radians(i)))), 5, (255,255,0), -1)

            cv2.putText(output, 'Leave a sparkling trail behind the index finger', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('55 Hand Sparkle Trail', output)
