    ...
    with mp_face.FaceMesh(max_num_faces=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as face_mesh:
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                nose = points[1]; chin = points[152]; left = points[234]; right = points[454]; overlay = output.copy(); cv2.rectangle(overlay, (left[0], left[1]-20), (right[0], chin[1]+20), (0,128,255), -1); output = cv2.addWeighted(output, 0.6, overlay, 0.4, 0)

            cv2.putText(output, 'Place a virtual mask overlay on detected faces', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('59 Face Mask Sticker', output)
