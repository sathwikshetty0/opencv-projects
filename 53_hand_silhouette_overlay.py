    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                mask = np.zeros_like(output); cv2.fillPoly(mask, [np.array([thumb_pos, index_pos, (index_pos[0], index_pos[1]+80), (thumb_pos[0], thumb_pos[1]+80)])], (0,255,0)); output = cv2.addWeighted(output, 0.7, mask, 0.3, 0);

            cv2.putText(output, 'Draw a hand silhouette overlay in real time', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('53 Hand Silhouette Overlay', output)
