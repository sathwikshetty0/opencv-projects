    ...
    with mp_hands.Hands(max_num_hands=1, min_detection_confidence=0.7, min_tracking_confidence=0.7) as hands:
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                ...
                radius = int(np.clip(distance, 60, 140)); mask = np.zeros((h, w), dtype=np.uint8); cv2.circle(mask, midpoint, radius, 255, -1); blurred = cv2.GaussianBlur(output, (31,31), 0); output = np.where(mask[:,:,None] == 255, output, blurred)

            cv2.putText(output, 'Warp a circular window around your hand', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(output, 'Press q to quit', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            cv2.imshow('58 Hand Warp Window', output)
